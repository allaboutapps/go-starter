...
type userR struct {
	AppUserProfile      *AppUserProfile
	AccessTokens        AccessTokenSlice
	PasswordResetTokens PasswordResetTokenSlice
	RefreshTokens       RefreshTokenSlice
...
var (
	...
	userColumnsWithoutDefault = []string{"id", "username", "password", "salt", "google_info", "facebook_info", "created_at", "updated_at"}
	userColumnsWithDefault    = []string{"google_id", "facebook_id", "is_active"}
...
func (o *User) UserPermissions(mods ...qm.QueryMod) userPermissionQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"user_permissions\".\"user_id\"=?", o.ID),
	)
...
func (userL) LoadAppUserProfile(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`app_user_profiles`), qm.WhereIn(`app_user_profiles.user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (userL) LoadAccessTokens(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`access_tokens`), qm.WhereIn(`access_tokens.user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (userL) LoadPasswordResetTokens(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`password_reset_tokens`), qm.WhereIn(`password_reset_tokens.user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (userL) LoadRefreshTokens(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`refresh_tokens`), qm.WhereIn(`refresh_tokens.user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (userL) LoadUserPermissions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`user_permissions`), qm.WhereIn(`user_permissions.user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (userL) LoadUserRoles(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`user_roles`), qm.WhereIn(`user_roles.user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
// Sets related.R.User appropriately.
func (o *User) AddUserPermissions(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserPermission) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
